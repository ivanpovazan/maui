...
After:
			if (serviceProvider?.GetService<IProvideValueTarget>() is XamlValueTargetProvider valueTargetProvider)
			{
				valueTargetProvider.TargetProperty = null;
*/
...
After:
			...
			exception = bindable.GetValue(property);
*/
...
*/
			...
			value = new XamlParseException($"{elementType.Name} is not a BindableObject or does not support setting native BindableProperties", lineInfo);
			return false;
		}
